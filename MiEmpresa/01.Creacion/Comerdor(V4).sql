...
-- 1.- Dirección quiere saber que tutores pagaron mas de 150 de menor a mayor
SELECT t.nombre as Nombre, t.apellido_paterno as ApellidoPaterno, 
t.apellido_materno as ApellidoMaterno, p.total as Total
FROM Tutor t
	JOIN Pago p on t.id_tutor = p.id_tutor
	WHERE p.total > 150
	ORDER BY p.total ASC;

-- 2.- Cocina quiere saber que comidas se van a realizar la segunda semana de abril
SELECT rc.nombre_comida FROM Semana s
	...
	WHERE s.fecha_inicio >= '2025-04-07' AND s.fecha_fin <= '2025-04-13';

-- 3.- Cocina quiere saber que comidas son las mas pedidas
SELECT c.nombre_comida as Comida, COUNT(*) as VecesPedida FROM Consumo co
	JOIN Menu m on co.id_menu = m.id_menu
	JOIN Comida c on m.id_comida = c.id_comida
	GROUP BY c.nombre_comida
	ORDER BY VecesPedida DESC;

-- 4.- Cocina quiere saber la receta de la comida: tacos al pastor
SELECT nombre_comida as Comida, procedimiento FROM Receta_Comida
	WHERE nombre_comida = 'tacos al pastor';

-- 5.- El chef quiere saber la suma de las porciones de los postres: tres leches y galletas de avena
SELECT SUM(porciones) as total_porciones FROM Receta_Postre
	WHERE nombre_postre in ('tres leches', 'galletas de avena');


-- 6.- Dirección quiere saber que tutores han pagado el mes de abril
...
-- 7.- cocina quiere saber las bebidas que se prepararon el primero de enero
...
-- 8.- La nutrióloga necesita saber cuantas comidas comieron los alumnos el dia 1 de septiembre del 2024
SELECT SUM(c.cantidad_porciones) as ComidasTotales FROM Consumo c
	JOIN Semana s on c.id_semana = s.id_semana
	WHERE s.fecha_inicio = '2024-09-01'

-- 9.- Cocina quiere saber que ingredientes va a ocupar el dia 4 del mes de junio
SELECT i.nombre_ingrediente FROM Compra c
	JOIN Ingrediente i ON c.id_ingrediente = i.id_ingrediente
	JOIN Semana s ON c.id_semana = s.id_semana
	WHERE s.fecha_inicio >= '2025-06-24'
	GROUP BY i.nombre_ingrediente

-- 10.- Dirección necesita saber que tutores han pagado menos de 130
SELECT t.nombre as Nombre, t.apellido_paterno as ApellidoPaterno, 
t.apellido_materno as ApellidoMaterno, p.total as Total
FROM Tutor t
	JOIN Pago p on t.id_tutor = p.id_tutor
...
-- 11.- Cocina quiere saber cuantas bebidas se van a hacer el día viernes
SELECT SUM(rb.porciones) as BebidasTotales FROM Receta_Bebida rb
	JOIN Bebida b on rb.id_receta_bebida = b.id_receta_bebida
	JOIN Menu m on b.id_bebida = m.id_bebida
	JOIN Semana s on m.id_menu = s.id_menu
	JOIN DiaSemana ds on s.id_semana = ds.id_diaS
	WHERE ds.diaS = 'Viernes';

-- 12.- El chef quiere el procedimiento que se necesita para hacer sopa de tortilla
SELECT nombre_comida, procedimiento FROM Receta_Comida
	WHERE nombre_comida in ('sopa de tortilla', 'pozole rojo');

-- 13.- El chef quiere saber el procedimiento de la bebida batido de fresa y las porciones que rinde
SELECT nombre_bebida, procedimiento, porciones FROM Receta_Bebida 
	WHERE nombre_bebida = 'batido de fresa';

-- 14.- Dirección necesita saber que productos se van a comprar en el mes de marzo
SELECT i.nombre_ingrediente, c.cantidad_necesaria, c.fecha_compra FROM Compra c
	JOIN Ingrediente i on c.id_ingrediente = i.id_ingrediente
	WHERE MONTH(c.fecha_compra) = 3 AND YEAR(c.fecha_compra) = '2025';

-- 15.- Dirección quiere saber que comidas comen los niños nacidos el año 2007
...
FROM Nino n
	...
	JOIN Comida c on m.id_comida = c.id_comida
	JOIN Receta_Comida rc on c.id_receta_comida = rc.id_receta_comida
	WHERE YEAR(n.fecha_nacimiento) = 2007;

-- 16.- Dirección necesita saber que tutores y que cantidad pagaron en el mes de agosto del 2025
...
-- 17.- Cocina quiere saber la cantidad que se requiere comprar del ingrediente queso parmesano el 2025-04-22
SELECT i.nombre_ingrediente, c.cantidad_necesaria FROM Compra c
	JOIN Ingrediente i on c.id_ingrediente = i.id_ingrediente
	WHERE i.id_ingrediente = 16 AND c.fecha_compra = '2025-04-22';

-- 18.- El chef quiere saber el menú que se hará la semana 3 del mes de abril
SELECT c.nombre_comida, b.nombre_bebida, p.nombre_postre
FROM Menu m
	JOIN Semana s on m.id_menu = s.id_menu
	JOIN Comida c on m.id_comida = c.id_comida
	JOIN Bebida b on m.id_bebida = b.id_bebida
	JOIN Postre p on m.id_postre = p.id_postre
	WHERE s.fecha_inicio >= '2025-04-14' AND s.fecha_fin <= '2025-04-20';

-- 19.- Dirección necesita saber el monto total de pagos que se han realizado el tutor Carlos
SELECT t.nombre, SUM(p.total) as TotalPagado FROM Pago p
	JOIN Tutor t on p.id_tutor = t.id_tutor
	WHERE t.nombre like 'Carlos%'
	GROUP BY t.nombre

-- 20.- Cocina quiere saber que postres no se van a preparar el 2025-05-15
SELECT rp.nombre_postre FROM Receta_Postre rp
	WHERE rp.id_receta_postre NOT IN (
		SELECT mp.id_receta_postre
		FROM Menu_Postre mp
		JOIN Semana s on mp.id_menuP = s.id_menu
		JOIN DiaSemana ds on s.id_semana = ds.id_diaS
		WHERE s.fecha_inicio = '2025-05-15');

-- 21.- El chef quiere saber el procedimiento de las comidas que rindan 5 porciones
SELECT nombre_comida, procedimiento, porciones FROM Receta_Comida
	WHERE porciones = 5;

-- 22.- Dirección quiere saber que se compro el día 2025-04-14
SELECT i.nombre_ingrediente as Ingrediente, c.cantidad_necesaria as Cantidad FROM Compra c
	JOIN Ingrediente i on c.id_ingrediente = i.id_ingrediente
	WHERE c.fecha_compra = '2025-04-14';

-- 23.- El chef quiere saber el promedio de porciones de las comidas
SELECT AVG(rc.porciones) as PromedioPorciones
	FROM Receta_Comida rc

-- 24.- Cocina quiere saber el platillo mas pedido de todos los tiempos
SELECT rc.nombre_comida, COUNT(*) as VecesPedido FROM Consumo co
	JOIN Menu m on co.id_menu = m.id_menu
	JOIN Comida c on m.id_comida = c.id_comida
	JOIN Receta_Comida rc on c.id_receta_comida = rc.id_receta_comida
	GROUP BY rc.nombre_comida
	ORDER BY VecesPedido DESC


-- 25.- Dirección requiere saber los tutores que su pago esta entre 130 y 170
SELECT t.nombre, t.apellido_paterno, t.apellido_materno, p.total FROM Tutor t
	JOIN Pago p on t.id_tutor = p.id_tutor
	WHERE p.total BETWEEN 130 AND 170;

-- 26.- Cocina requiere de saber el platillo que contiene cebolla
SELECT nombre_comida, procedimiento FROM Receta_Comida
	WHERE procedimiento like '%cebolla%'

-- 27.- El chef quiere saber el procedimiento de las bebidas que son jugos
SELECT nombre_bebida, procedimiento FROM Receta_Bebida
	WHERE nombre_bebida like '%Jugo%'

-- 28.- Dirección quiere saber la cantidad de dinero ganada durante los meses de enero, febrero y abril
SELECT SUM(p.total) as Ganancias FROM Pago p
	WHERE MONTH(p.fecha_pago) in (1, 2, 4) AND YEAR(p.fecha_pago) = '2025';

-- 29.- Cocina quiere saber que mes y año han tenido mayor demanda de platillos
SELECT MONTH(s.fecha_inicio) as Mes, YEAR(s.fecha_inicio) as Año, COUNT(co.id_menu) as Demanda
FROM Consumo co
	JOIN Semana s on co.id_semana = s.id_semana
	GROUP BY MONTH(s.fecha_inicio), YEAR(s.fecha_inicio)
	ORDER BY Demanda DESC

-- 30.- Cocina quiere saber una comida que rinda 5 poriones e incluya pollo en su preparacion
SELECT nombre_comida, porciones FROM Receta_Comida
	WHERE porciones = 5 and procedimiento like '%pollo%';