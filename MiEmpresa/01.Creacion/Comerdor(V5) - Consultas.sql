...
INSERT INTO Nino (nombre, apellido_paterno, apellido_materno, fecha_nacimiento, grado, nivel, alergias_alimenticias, id_tutor) VALUES
    ('Pedro', 'Perez', 'Gomez', '2018-03-28', 'Primero', 'Primaria', 'Ninguna', 1),
    ('Juan', 'Ramírez', 'López', '2015-06-15', 'Cuarto', 'Primaria', 'Ninguna', 2),
    ('Ana', 'Martínez', 'López', '2016-06-18', 'Tercero', 'Primaria', 'Camarón', 3),
    ('Luis', 'Gómez', 'Vega', '2017-05-14', 'Segundo', 'Primaria', 'Ninguna', 4),
    ('María', 'Hernández', 'López', '2018-08-30', 'Primero', 'Primaria', 'Frutos secos', 5),
    ('Sofía', 'Díaz', 'Mendoza', '2013-01-12', 'Quinto', 'Primaria', 'Ninguna', 6),
    ('Carlos', 'Fernández', 'Ruiz', '2012-09-25', 'Sexto', 'Primaria', 'Gluten', 7),
    ('Valeria', 'Ortega', 'Sánchez', '2015-11-05', 'Cuarto', 'Primaria', 'Lácteos', 8),
    ('Javier', 'López', 'Castro', '2016-07-18', 'Tercero', 'Primaria', 'Ninguna', 9),
    ('Elena', 'Jiménez', 'Ortiz', '2017-10-22', 'Segundo', 'Primaria', 'Maní', 10),
    ('Diego', 'Vargas', 'Reyes', '2018-02-14', 'Primero', 'Primaria', 'Ninguna', 11),
    ('Camila', 'Suárez', 'Morales', '2013-04-19', 'Quinto', 'Primaria', 'Mariscos', 12),
    ('Mateo', 'Navarro', 'Silva', '2015-06-30', 'Cuarto', 'Primaria', 'Ninguna', 13),
    ('Lucía', 'Cruz', 'Pérez', '2012-12-05', 'Sexto', 'Primaria', 'Ninguna', 14),
    ('Emilio', 'Torres', 'Delgado', '2016-08-07', 'Tercero', 'Primaria', 'Huevo', 15),
    ('Regina', 'Gómez', 'Méndez', '2017-09-15', 'Segundo', 'Primaria', 'Ninguna', 1),
    ('Andrés', 'Hernández', 'Ríos', '2018-03-03', 'Primero', 'Primaria', 'Lácteos', 2),
    ('Natalia', 'Rodríguez', 'García', '2013-07-22', 'Quinto', 'Primaria', 'Cacahuates', 3),
    ('Manuel', 'Pérez', 'Flores', '2014-05-28', 'Cuarto', 'Primaria', 'Ninguna', 4),
    ('Victoria', 'Santos', 'Guzmán', '2018-11-11', 'Primero', 'Primaria', 'Fresas', 5);
...
-- 1.- El chef quiere saber la suma de las porciones de los postres: tres leches y galletas de avena
SELECT SUM(porciones) as total_porciones FROM Receta_Postre
	WHERE nombre_postre in ('tres leches', 'galletas de avena');


-- 2.- Direcci�n quiere saber que tutores han pagado el mes de abril
SELECT DISTINCT t.nombre, t.apellido_paterno, t.apellido_materno, p.total, p.fecha_pago
FROM Tutor t
	JOIN Pago p on t.id_tutor = p.id_tutor
	WHERE MONTH(p.fecha_pago) = 4 AND YEAR(p.fecha_pago) = YEAR(GETDATE());


-- 3.- Cocina quiere saber las bebidas que se prepararon el primero de enero
SELECT DISTINCT rb.nombre_bebida FROM Semana s
	JOIN Menu_Bebida mb on s.id_menu = mb.id_menuB
	JOIN Receta_Bebida rb on mb.id_receta_bebida = rb.id_receta_bebida
	WHERE s.fecha_inicio >= '2025-01-01'

-- 4.- La nutri�loga necesita saber cuantas comidas comieron los alumnos el dia 1 de septiembre del 2024
SELECT SUM(c.cantidad_porciones) as ComidasTotales FROM Consumo c
	JOIN Semana s on c.id_semana = s.id_semana
	WHERE s.fecha_inicio = '2024-04-01'

-- 5.- El chef quiere saber el promedio de porciones de las comidas
SELECT AVG(rc.porciones) as PromedioPorciones
	FROM Receta_Comida rc

-- 6.- Mostrar el promedio de porciones consumidas por semana 
SELECT s.id_semana, AVG(porciones) AS PromedioPorciones
FROM (
  SELECT id_semana, SUM(cantidad_porciones) AS porciones
  FROM Consumo
  GROUP BY id_semana, id_nino
) AS PorcionesPorNinoSemana
JOIN Semana s ON PorcionesPorNinoSemana.id_semana = s.id_semana
GROUP BY s.id_semana;

-- 7.- Direección quiere alcular días entre fechas dee la semana 1
WITH Dias AS (
  ...
  FROM Dias
  WHERE Fecha < (SELECT fecha_fin FROM Semana WHERE id_semana = 1)
)
SELECT Fecha FROM Dias OPTION (MAXRECURSION 1000);

-- 8.- Listar los ingredientes y la cantidad total usada en todas las recetas
SELECT i.nombre_ingrediente, SUM(ri.cantidad) AS CantidadTotalUsada
FROM Receta_Ingrediente ri
JOIN Ingrediente i ON ri.id_ingrediente = i.id_ingrediente
GROUP BY i.nombre_ingrediente;

-- 9. Cocina quiere saber cual es el número de recetas de cada tipo
SELECT t.nombre_tipo, COUNT(r.id_receta) AS NumeroRecetas
FROM Receta r
JOIN Tipo t ON r.id_tipo = t.id_tipo
GROUP BY t.nombre_tipo;

-- 10.- Direccion quiere saber el número de teléfonos disponibles de tutores (teléfono fijo y celular)
SELECT nombre, apellido_paterno,
       COALESCE(telefono_celular, telefono, 'Sin teléfono') AS TelefonoDisponible
FROM Tutor;

-- 11.- Direccion quiere saber las fechas de las semanas que incluyen el menú con mayor número de consumos
SELECT s.fecha_inicio, s.fecha_fin, s.id_menu FROM Semana s
    WHERE s.id_menu = (
...
-- 12.- Direccion quiere clasificar el nivel de consumo semanal por niño
SELECT n.nombre,
       SUM(c.cantidad_porciones) AS TotalPorciones,
       CASE 
         WHEN SUM(c.cantidad_porciones) < 3 THEN 'Bajo'
         WHEN SUM(c.cantidad_porciones) BETWEEN 3 AND 6 THEN 'Medio'
         ELSE 'Alto'
       END AS NivelConsumo
FROM Consumo c
...
-- 13.- Direccion quiere saber el nombre del Tutor y su total pagado para cada semana 
SELECT t.nombre, t.apellido_paterno, s.id_semana, SUM(p.total) AS TotalPagado FROM Pago p
    JOIN Tutor t ON p.id_tutor = t.id_tutor
    JOIN Semana s ON p.id_semana = s.id_semana
    GROUP BY t.nombre, t.apellido_paterno, s.id_semana
    ORDER BY s.id_semana;

-- 14.- Direccion quiere saber el total necesario de compras por ingrediente para la semana 1
WITH ComprasSemana AS (
  SELECT id_ingrediente, SUM(cantidad_necesaria) AS TotalNecesario
  FROM Compra
  WHERE id_semana = 1
  GROUP BY id_ingrediente
)
SELECT i.nombre_ingrediente, cs.TotalNecesario
FROM ComprasSemana cs
JOIN Ingrediente i ON cs.id_ingrediente = i.id_ingrediente;

-- 15.- Cocina quiere saber cual es el consumo diario de porciones por día de la semana
SELECT DS.id_diaS, DS.diaS, COUNT(*) AS total_consumos FROM Consumo C
    JOIN DiaSemana DS ON C.id_diaS = DS.id_diaS
    GROUP BY DS.id_diaS, DS.diaS
    ORDER BY DS.id_diaS;

-- 16.- Cocina quiere saber el total de ingredientes distintos usados en todas las recetas
SELECT COUNT(DISTINCT id_ingrediente) AS IngredientesUsados
FROM Receta_Ingrediente;

-- 17.- Cocina quiere saber el numero de menús y el número de recetas que hay disponibles por para tipo
SELECT m.id_menu, 
  (SELECT COUNT(*) FROM Menu_Comida mc WHERE mc.id_menuC = m.id_menu) +
  (SELECT COUNT(*) FROM Menu_Bebida mb WHERE mb.id_menuB = m.id_menu) +
  (SELECT COUNT(*) FROM Menu_Postre mp WHERE mp.id_menuP = m.id_menu) AS TotalRecetas
FROM Menu m;

-- 18.- Direccion quiere saber ¿Qué ingredientes se utilizan más por receta?
SELECT I.nombre_ingrediente, COUNT(*) AS veces_usado
FROM Receta_Ingrediente RI
    JOIN Ingrediente I ON RI.id_ingrediente = I.id_ingrediente
GROUP BY I.nombre_ingrediente
ORDER BY veces_usado DESC;

-- 19.- Alergias que tengan los niños
SELECT 
    nombre, 
    apellido_paterno, 
    COALESCE(alergias_alimenticias, 'Ninguna') AS alergias
FROM Nino;

-- 20.- Total de pago por tutor en la semana 1 (si no ha pagado, muestra 0):
SELECT 
    T.nombre, 
    T.apellido_paterno, 
    S.fecha_inicio, 
...
FROM Tutor T
JOIN Semana S ON S.id_semana = 1
LEFT JOIN Pago P ON P.id_tutor = T.id_tutor AND P.id_semana = S.id_semana;

-- 21.- Ingrediente con su unidad de medida (si no hay unidad pone: "N/A")
SELECT 
    I.nombre_ingrediente,
    COALESCE(U.unidadMedida, 'N/A') AS unidad
FROM Ingrediente I
LEFT JOIN Receta_Ingrediente RI ON I.id_ingrediente = RI.id_ingrediente
LEFT JOIN Unidad U ON RI.id_unidad = U.id_unidad;

-- 22. Menú consumido por niño por día (Semana 1)
SELECT * 
FROM (
    SELECT 
        N.nombre + ' ' + N.apellido_paterno + ' ' + N.apellido_materno AS nombre_nino,
        D.diaS,
        RC.nombre_comida
    FROM Consumo C
    JOIN Nino N ON C.id_nino = N.id_nino
    JOIN DiaSemana D ON C.id_diaS = D.id_diaS
    JOIN Menu M ON C.id_menu = M.id_menu
    JOIN Comida C2 ON M.id_comida = C2.id_comida
    JOIN Receta_Comida RC ON C2.id_receta_comida = RC.id_receta_comida
    WHERE C.id_semana = 1
) AS datos
PIVOT (
    MAX(nombre_comida) FOR diaS IN ([Lunes], [Martes], [Miercoles], [Jueves], [Viernes], [Sabado], [Domingo])
) AS tabla_pivot;

-- 23.- Mostrar si el niño tiene alergias o no (pero ahora con CASE y sin especificar cual es)
SELECT 
    nombre,
    apellido_paterno,
    CASE 
        WHEN alergias_alimenticias IS NULL OR alergias_alimenticias = 'Ninguna' THEN 'Sin alergias'
        ELSE 'Con alergias'
    END AS estado_alergia
FROM Nino;

-- 24.- Direccion quiere saber que cantidad de compras debe de hacer por ingrediente en cada semana
SELECT * 
FROM (
    SELECT 
        I.nombre_ingrediente, 
...
PIVOT (
    SUM(cantidad_necesaria) FOR id_semana IN ([1], [2], [3])
) AS pivot_result;

-- 25.- Direccion quiere los nombres completos de los niños y su edad exacta pero quiere que los nombres esten en mayusculas
SELECT 
    UPPER(CONCAT(n.nombre, ' ', n.apellido_paterno, ' ', n.apellido_materno)) AS nombre_completo,
    DATEDIFF(YEAR, n.fecha_nacimiento, GETDATE()) 
        - CASE 
            WHEN MONTH(n.fecha_nacimiento) > MONTH(GETDATE()) 
                 OR (MONTH(n.fecha_nacimiento) = MONTH(GETDATE()) AND DAY(n.fecha_nacimiento) > DAY(GETDATE()))
            THEN 1 
            ELSE 0 
          END AS edad_exacta
FROM Nino n;

-- 26.- Direccion quiere saber el año y mes de cada compra
SELECT 
    nombre_ingrediente,
...
-- 27.- Direccion quiere hacer una clasificación del pago por tutor, saber si ha pagado o no
SELECT 
    T.nombre + ' ' + T.apellido_paterno AS tutor,
    CASE 
        WHEN P.total IS NULL THEN 'No ha pagado'
        WHEN P.total < 500 THEN 'Pago bajo'
        WHEN P.total BETWEEN 500 AND 1000 THEN 'Pago medio'
        ELSE 'Pago alto'
    END AS estado_pago
FROM Tutor T
LEFT JOIN Pago P ON T.id_tutor = P.id_tutor AND MONTH(P.fecha_pago) = 4;

-- 28.- Direccion quiere saber el total de pagos por tutor en el mes de abril
WITH PagosAbril AS (
    SELECT id_tutor, total
    FROM Pago
    WHERE MONTH(fecha_pago) = 4
)
SELECT T.nombre, T.apellido_paterno, SUM(P.total) AS total_pagado
FROM Tutor T
JOIN PagosAbril P ON T.id_tutor = P.id_tutor
GROUP BY T.nombre, T.apellido_paterno;

-- 29.-  Direcci�n quiere saber que comidas comen los ni�os nacidos el a�o 2017
SELECT rc.nombre_comida, n.nombre, n.apellido_paterno, n.apellido_materno, n.fecha_nacimiento
FROM Nino n
	JOIN Consumo co on n.id_nino = co.id_nino
	JOIN Menu m on co.id_menu = m.id_menu
	JOIN Comida c on m.id_comida = c.id_comida
	JOIN Receta_Comida rc on c.id_receta_comida = rc.id_receta_comida
	WHERE YEAR(n.fecha_nacimiento) = 2017;

-- 30.- Direcci�n necesita saber que tutores y que cantidad pagaron en el mes de agosto del 2025
SELECT t.nombre, t.apellido_paterno, t.apellido_materno, p.total FROM Tutor t
	JOIN Pago p on t.id_tutor = p.id_tutor
	WHERE MONTH(p.fecha_pago) = 8 AND YEAR(p.fecha_pago) = 2025;
